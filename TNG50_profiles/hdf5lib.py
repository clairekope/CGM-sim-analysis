...
def OpenFile(fname, mode = "r"):
	if (use_tables):
		return tables.open_file(fname, mode = mode)
	else:
		return h5py.File(fname, mode)

def GetData(f, dname):
	if (use_tables):
		return f.root._f_get_child(dname)
	else:
		return f[dname]

def GetGroup(f, gname):
	if (use_tables):
		return f.root._f_get_child(gname) 
	else:
		return f[gname]

def GetAttr(f, gname, aname):
	if (use_tables):
		return f.root._f_get_child(gname)._f_getattr(aname) 
	else:
...
def Contains(f, gname, cname):
	if (use_tables):
		if gname=="":
			...
			return f.root._f_get_child(gname).__contains__(cname)
	else:
		if gname=="":
...
def CreateArray(f, where, aname, aval):
	if (use_tables):
		f.create_array(where, aname, aval)
	else:
